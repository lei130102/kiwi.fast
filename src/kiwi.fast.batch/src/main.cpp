...
int wmain(int argc, wchar_t* argv[], wchar_t* envp[])
{
    ...
    options_description_.add_options()
            //cpu_mask
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"cpu_mask").c_str()
             , boost::program_options::wvalue<int>()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"cpu mask").c_str())
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"name").c_str()
             , boost::program_options::wvalue<std::wstring>()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"name").c_str())
            //year
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"year").c_str()
             , boost::program_options::wvalue<int>()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"年份").c_str())
            //month
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"month").c_str()
             , boost::program_options::wvalue<int>()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"月份").c_str())
            //element
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"element").c_str()
             , boost::program_options::wvalue<int>()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"元素类型：0-温度 1-盐度").c_str())
            //interval
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"interval").c_str()
             , boost::program_options::wvalue<int>()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"分辨率").c_str())
            //sqlite_filepath
            (KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"sqlite_filepath").c_str()
             , boost::program_options::wvalue<std::vector<std::wstring>>()->zero_tokens()->multitoken()->composing()
             , KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"sqlite数据库文件路径").c_str())
            //
    ...
    */
    ...
    if(is_debug)
    {
        //测试
        cpu_mask = 0x00000002;
        name = KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8"");
        year = 1999;
        month = 6;
        element = 0;
        interval = 2;

        sqlite_filepath.push_back(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8R"(F:\DQ1028Data\海洋水文资料\海洋温盐\国际业务化资料\信息中心\历史\Basic\S1000-Y1999-TS.dat.db)"));
        sqlite_filepath.push_back(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8R"(F:\DQ1028Data\海洋水文资料\海洋温盐\国际业务化资料\信息中心\历史\Basic\S1004-Y1999-TS.dat.db)"));
        sqlite_filepath.push_back(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8R"(F:\DQ1028Data\海洋水文资料\海洋温盐\国际业务化资料\信息中心\历史\Basic\S1005-Y1999-TS.dat.db)"));
    }
    else
    {
        //cpu_mask
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"cpu_mask")))
        {
        ...
        //name
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"name")))
        {
        ...
        name = variables_map_["name"].as<std::wstring>();
        BOOST_LOG_TRIVIAL(error) << "cpu_mask:" << cpu_mask;
        //year
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"year")))
        {
        ...
        //month
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"month")))
        {
        ...
        //element
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"element")))
        {
        ...
        //interval
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"interval")))
        {
        ...
        //sqlite_filepath
        if(!variables_map_.count(KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char>(u8"sqlite_filepath")))
        {
        ...
        for(auto const& filepath : sqlite_filepath)
        {
            ...
            for(int lat = 90; lat >= -90; lat -= interval.get())
            {
                for(int lon = -180; lon <= 180; lon += interval.get())
                {
                    ...
                    sql += KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char8_t>(std::to_wstring(lat - interval.get()));
                    sql += u8" AND ";
                    ...
                    sql += u8" AND ";
                    sql += KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char8_t>(std::to_wstring(lon + interval.get()));
                    sql += u8" THEN ";
                    sql += KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char8_t>(std::to_wstring(lat));
                }
            }
    ...
    else if(name == KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8"cengshen"))
    {
        ...
        for(auto const& filepath : sqlite_filepath)
        {
            ...
            while(sqlite3_step(sqlite_stmt) == SQLITE_ROW)
            {
                tmp_query_result.add(year.get(), month.get(), element.get(), interval.get()
    ...
    else if(name == KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8"tongcun"))
    {
        ...
        for(auto const& filepath : sqlite_filepath)
        {
            ...
            while(sqlite3_step(sqlite_stmt) == SQLITE_ROW)
            {
                tmp_query_result.add(year.get(), month.get(), interval.get(), sqlite3_column_double(sqlite_stmt, 0)
                                     , sqlite3_column_double(sqlite_stmt, 1)
                                     , sqlite3_column_int64(sqlite_stmt, 2)
                                     , sqlite3_column_int64(sqlite_stmt, 3));
            }
    ...
    else if(name == KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<wchar_t>(u8"tongguo"))
    {
        ...
        for(auto const& filepath : sqlite_filepath)
        {
            ...
            while(sqlite3_step(sqlite_stmt) == SQLITE_ROW)
            {
                tmp_query_result.add(year.get(), month.get(), element.get(), interval.get(), KIWI_FAST_PLUGIN_UTILITY_NAMESPACE_QUALIFIER code_conversion<char8_t>(sqlite3_column_text(sqlite_stmt, 2))
                                     , sqlite3_column_double(sqlite_stmt, 0)
                                     , sqlite3_column_double(sqlite_stmt, 1)
                                     , sqlite3_column_int64(sqlite_stmt, 3)
                                     , sqlite3_column_int64(sqlite_stmt, 4)
